...
func validateWriters(fs *pflag.FlagSet) {
	fs.Visit(func(f *pflag.Flag) {
		...
		case "role":
			...
			if _, ok := api.UserRoleLookup[v]; !ok {
		...
		case "resource-type":
			...
			if _, ok := api.UserResourceTypes[v]; !ok {
				panic("[validateWriters] invalid resource type")
			}
		case "alert-email-format":
			v, _ := fs.GetInt(f.Name)
			if _, ok := api.UserEmailFormats[v]; !ok {
		...
		case "alert-methods-down":
			...
			if v := lookup(d, api.UserNotificationMethods); v == nil {
		...
		case "alert-methods-trouble":
			...
			if v := lookup(t, api.UserNotificationMethods); v == nil {
		...
		case "alert-methods-up":
			...
			if v := lookup(u, api.UserNotificationMethods); v == nil {
		...
		case "alert-methods-applogs":
			...
			if v := lookup(al, api.UserNotificationMethods); v == nil {
